...
实现 Developer-Reviewer 自动迭代引擎，以及项目的睡眠/唤醒生命周期管理。
...
- `acceptanceCriteria[]` ← Reviewer Agent 的 `TodoWrite` 事件实时捕获，追加写入
...
- **Reviewer Agent**：审查本轮实现，逐项验收，判断里程碑是否整体完成
...
loop（直到里程碑完成或达到 max_iterations）:
  ...
  Reviewer Session 启动（conversationAgent）
  ...
  loop:
    round += 1
    Developer 发送本轮实现报告
    Reviewer 收到报告，创建验收 TodoList（Anima 捕获 → milestone.acceptanceCriteria，iteration=N）
    Reviewer 逐项核验，更新 TodoList 状态

    if Reviewer TodoList 全部 completed:
      if 里程碑所有需求已满足（Reviewer 判断）:
      ...
      if round >= max_rounds_per_iteration:
        ...
        将 Reviewer 的具体反馈发回 Developer（同一 session 追加消息）
...
- **Reviewer session** 的 `TodoWrite` → 追加到 `milestone.acceptanceCriteria`，`iteration` = 当前轮次，状态实时同步
...
- 左右双面板：分别展示 Developer 和 Reviewer 的输出流（活跃中的 Agent 高亮）
- 顶部状态栏：当前里程碑名称、当前迭代轮次、当前 battle round
- Task 列表：按 `iteration` 分组展示 Developer 的 TodoList 及完成状态
- 验收项列表：按 `iteration` 分组展示 Reviewer 的验收 TodoList 及通过状态
...
- [ ] 每次迭代各开一对新的 Developer / Reviewer session，迭代结束后关闭
- [ ] Developer 的 TodoWrite 事件被捕获，实时追加到 `milestone.tasks`，携带正确的 `iteration` 值
- [ ] Reviewer 的 TodoWrite 事件被捕获，实时追加到 `milestone.acceptanceCriteria`，携带正确的 `iteration` 值
- [ ] Reviewer TodoList 全部 completed 后，Reviewer 正确判断里程碑是否整体完成
- [ ] 里程碑整体完成时 `milestone.status → completed`，`state.json` 同步更新
- [ ] battle 内 Reviewer 的拒绝反馈正确传回 Developer（同一 session 追加消息）
